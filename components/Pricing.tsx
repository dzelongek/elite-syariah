    ...
    >
      ...
      <div className="relative h-56 w-full overflow-hidden bg-gray-200 group-hover:scale-[1.01] transition-transform duration-500">
        ...
        {plan.images.map((img, idx) => (
          <div
            key={idx}
            className={`absolute inset-0 w-full h-full transition-opacity duration-500 ease-in-out ${idx === currentImageIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'
              }`}
          >
            <img
              src={img}
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center mt-8">
            <div className="bg-white p-1.5 rounded-full shadow-md inline-flex relative border border-gray-200">
              <div
                className={`absolute top-1.5 bottom-1.5 rounded-full bg-emerald-900 transition-all duration-300 w-[140px] md:w-[160px] ${mode === 'daily' ? 'left-1.5' : 'left-[146px] md:left-[166px]'
                  }`}
              ></div>

              <button
                onClick={() => setMode('daily')}
                className={`relative z-10 w-[140px] md:w-[160px] py-3 rounded-full font-bold text-sm md:text-base transition-colors duration-300 flex items-center justify-center gap-2 ${mode === 'daily' ? 'text-white' : 'text-gray-500 hover:text-emerald-900'
                  }`}
              >
              ...
              <button
                ...
                className={`relative z-10 w-[140px] md:w-[160px] py-3 rounded-full font-bold text-sm md:text-base transition-colors duration-300 flex items-center justify-center gap-2 ${mode === 'monthly' ? 'text-white' : 'text-gray-500 hover:text-emerald-900'
                  }`}
              >
